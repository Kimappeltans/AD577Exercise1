...
target_language = st.selectbox("Choose target language", [
    "French", "Spanish", "German", "Russian", "Japanese", "Italian"  # Added Italian here
])
...
def load_translation_model(language):
    try:
        if language == "French":
            ...
            return pipeline("translation_en_to_ja", model="Helsinki-NLP/opus-mt-en-ja")
        elif language == "Italian":
            return pipeline("translation_en_to_it", model="Helsinki-NLP/opus-mt-en-it")  # Added Italian model
        else:
